docker run --name zookeeper --network kafka-test --network-alias zoo-net -p 22181:2181 -e ZOOKEEPER_CLIENT_PORT=2181 -e ZOOKEEPER_TICK_TIME=2000 -d confluentinc/cp-zookeeper:latest;
...
docker run --name postgres --network kafka-test --network-alias pg-net -e POSTGRES_PASSWORD=postgres -d -p 5432:5432 postgres:13.0;
...
docker run --name gateway-app --network kafka-test -d -p 8765:8765 f618db488fdb;