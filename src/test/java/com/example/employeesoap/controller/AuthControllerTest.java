...
class AuthControllerTest extends IntegrationTest
{
    ...
    void registerValidUserTest() {
        UserDto userDto = UserDto.builder()
                ...
                .password(PASSWORD)
                .build();
        ResponseEntity<UserDto> responseEntity = (ResponseEntity<UserDto>) authController.registerUser(userDto);
    ...
    void registerUserDuplicateUsernameTest() {
        ...
        ResponseEntity<UserDto> responseEntity = (ResponseEntity<UserDto>) authController.registerUser(userDto);
        assertSame(ERROR, responseEntity.getBody().getStatus());
        long count = userRepository.findAll().stream().filter(user -> user.getUsername().equals(USERNAME_DUPLICATE)).count();
        assertSame(count, 1L);
    }
    ...
    void registerUserDuplicateEmailTest() {
        ...
        ResponseEntity<UserDto> responseEntity = (ResponseEntity<UserDto>) authController.registerUser(userDto);
        assertSame(ERROR, responseEntity.getBody().getStatus());
        long count = userRepository.findAll().stream().filter(user -> user.getEmail().equals(EMAIL_DUPLICATE)).count();
        assertSame(count, 1L);
    }
    ...
    void authValidUserWithoutRolesTest() {
        ...
        ResponseEntity<JwtResponse> responseEntity = (ResponseEntity<JwtResponse>) authController.authUser(loginRequest);
